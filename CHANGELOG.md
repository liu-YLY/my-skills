...
- 历史用例与旧评测报告保持原样，已安装个人 Skills 不自动覆盖；验证和模型对照结果见本次交付报告。
...
- test-case-engineer 升级到 v8.6.0：新增「用例编排与排序规范」——默认策略一按用户旅程编排（入口 → 初始状态 → 创建 → 查询 → 编辑 → 删除 → 恢复 → 辅助操作，阶段内正向先行、同级按优先级）；需求庞大时策略二按功能内聚性模块化分组（模块间依赖拓扑排序、组内按用户旅程）；含策略决策规则、通用排序规则与 ID 联动约定；阶段 3 CHECKPOINT 与阶段 4 自检清单新增编排检查项
