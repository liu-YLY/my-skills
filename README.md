...
> 基于 AI 的智能技能系统，提供测试策略、测试用例生成、性能测试、Bug 根因分析、状态机驱动的状态型需求测试、变更影响分析、微信公众号排版等专业技能
...
> - **testing-bundle plugin**：testing-bundle + test-strategy-engineer + test-case-engineer + performance-test-engineer + bug-analyzer + state-machine-test-engineer（位于 `plugins/testing/skills/`），可选配套 state-machine-testing MCP Server（位于 `plugins/testing/mcp-servers/state-machine-testing/`）
...
| 混合意图（5 条链） | 见 SKILL.md 混合意图链 | 转交点 🔴 CHECKPOINT |
