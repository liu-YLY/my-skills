...
**适用场景**：优化循环（darwin-skill 场景）必算；准入评级场景推荐算（若 B3.6 dry_run，效果分仅取 B1+B3+B4，标注"未含 baseline 对照"）。
...
- 例外：若新增内容是必要的反例/边界条款/降级表（A5/A4.3 维度），不算冗余
...
```
触发条件：paired 比较中 margin=slight 占比 > 60% 且 better 数 = N
```

- 触发含义：judge 普遍认为"改后版略好"但都是 slight margin——可能是乐观偏差而非真实改进
- 处理：触发 §7"改后版潜在退化场景必填"项的强制执行；若 judge 列不出退化场景，该票作废重投
- 升级：close call 升 N=5 重投，若仍 slight 占比 > 60%，判定为"持平"而非"改进"
