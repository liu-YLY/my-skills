...
| v1.2 | 2026-08-06 | 针对"得分虚高但实际效果未提升"问题：①§5 新增效果子评级（B1+B3+B4+B3.6 增益）独立展示 ②B3.6 从"可选"升级为"推荐" ③§7 paired 比较增加"改后版潜在退化场景"必填项 ④新增 §12 无效改进识别章节（6 条检测规则 + 软封顶机制） ⑤§9 优缺点框架加"疑似无效改进"显式标注要求 ⑥§10 报告结构加效果子评级与无效改进检测字段 |
| v1.3 | 2026-08-06 | 针对 v1.2 实施过程中暴露的 4 个执行细节缺陷：①§7 paired 退化场景明确"两要素"质量标准与降权逻辑（票降权为 tie，连续 2 轮列不出→该轮退为 tie）②§5.4 + §12.4 明文化 dry_run 降级算法（B1+B3+B4，不加 B3.6 增量）与可信度三档标识（高/中/低）③§12 规则 5 触发条件从 `better=N` 放宽为 `better 占比 ≥ 2/3`（覆盖 2B+1T / 2B+1W 典型乐观偏差场景）④§12 规则 2 例外判定改为可追溯标准（必须可追溯到 A4.3/B4.2/A5/B5.4 评分项） |
| v1.4 | 2026-08-06 | 针对 test-case-engineer 测评中 A3.3 模糊词触发封顶暴露的 3 个判定缺陷：①A3.3 词性例外细化——明确"动作指引+具体动作"的判定标准（动词+可执行宾语+触发条件三要素）②新增"配套例外判定的软约束"豁免（rule with escape hatch，须含主约束+例外条款+判定条件三层结构）③阈值从绝对计数改为密度阈值（按知识库总行数归一化：<500 行用绝对计数 / ≥500 行用密度 ≥3/千行，避免大 skill 不公平） |
...
| A3.3 **模糊词黑名单**（v1.1+，v1.4 修订） | — | 可执行具体性子项（darwin-skill dim5 启发）：扫描知识文件与入口文件，**禁止出现**"建议/可以考虑/根据情况/灵活把握/视情况而定/酌情处理/视实际场景"等**软化措辞**（指作为指令修饰语、降低执行确定性的用法）。该黑名单来自 SkillLens actionable specificity 维度的实证：模糊措辞让 LLM 无法执行具体动作。判定时需区分"软化的指令"vs"具体的行动指引"。<br><br>**触发阈值**（v1.4 改为密度阈值，避免大 skill 不公平）：<br>• 知识库总行数 < 500 行：用绝对计数——软化措辞 ≥3 处 → A3.3 封顶 1 分；≥6 处 → 0 分<br>• 知识库总行数 ≥ 500 行：用密度阈值——软化措辞密度 ≥ 3/千行 → A3.3 封顶 1 分；密度 ≥ 6/千行 → 0 分<br>• 密度 = 软化措辞处数 / (知识库总行数 / 1000)<br><br>**词性例外**（v1.4 细化判定标准）：<br>① **名词例外**："建议/考虑"作为**名词**（如"修复建议""改进建议""处置建议"作为输出字段名/章节标题/表格列名）不算软化，属专业术语。判定：删除该词后句子是否仍通顺——通顺则属名词例外<br>② **动作指引例外**（v1.4 三要素细化）："建议/可以考虑"作为**指向用户的动作指引**不算软化，须**同时满足三要素**：(a) 含**动词**（如"补充""增加""检查"）(b) 含**可执行宾语**（如"边界用例""trace_id""索引"——非"相关内容""必要措施"等抽象宾语）(c) 含**触发条件或场景**（如"在异常路径""当 P0 占比 > 30% 时"）。三要素缺一即算软化。例：✅"建议在异常路径补充 trace_id"（动词+具体宾语+场景，三要素齐全）❌"建议补充相关测试"（宾语抽象，缺要素 b）❌"建议考虑优化"（无具体宾语，缺要素 b）<br>③ **纯软化修饰**（v1.4 明确）："可以考虑 X"**无配套具体参数/步骤/阈值**才算软化。例：❌"可以考虑用 Redis"（无配置/容量/TTL 参数）；✅"可以考虑用 Redis，TTL 设 5min，key 格式 `idem:{request_id}`"（有具体参数，归动作指引例外 ②）<br>④ **启发式段落例外**：在显式标注的"启发式指引"段落或"开放任务"步骤中允许少量出现，须声明"此处为高自由度指引"<br>⑤ **配套例外判定的软约束豁免**（v1.4 新增）：若软化措辞属于"**rule with escape hatch**"——即主约束 + 例外条款 + 判定条件三层结构齐全——不算软化。判定三要素：(a) 有**主约束**（如"title 建议 40 字符以内"）(b) 有**例外条款**（如"超长时按 3 条例外判定处理"）(c) 例外条款含**具体判定条件**（如"模块名超长允许截断+省略号""复合功能允许 60 字符"）。三层结构齐全则该软化措辞豁免不计入计数。例：✅ test-standards.md "title 建议 40 字符以内（非硬性约束，超长按 3 条例外判定处理：①模块名超长 ②复合功能 ③多状态联动）"——三层齐全，豁免；❌"建议 40 字符以内"无例外条款——不豁免 |
...
#### B3.6 Baseline 对照（v1.2 起从"可选"升级为"推荐"，不计入 10 分基数）

**为什么单列**：darwin-skill dim8 "实测表现"采用 with_skill vs baseline 对照，能证明 skill 是否真正带来增益（而非模型本身能力）。但本方案的 B3 评的是"输出质量绝对值"，with/without 对照是另一种评法，纳入会改变 10 分制结构。故作为**推荐增强观察项**，不计入 B3 小计，但在 §5.4 效果子评级与 §12 无效改进识别中起决定性作用。
...
3. 结果记录为 `增益等级` + `量化增量`（v1.2 新增）：
   - `增益等级`：{显著增益 / 持平 / 退步}
   - `量化增量`（v1.2）：显著增益 = +3，持平 = +1，退步 = -3（用于 §5.4 效果子评级换算）
   - `对照 prompt 数` + `dry_run 比例`

**适用场景**（v1.2 调整）：
- 高风险 skill（影响生产决策）**强制**跑 baseline 对照
- 优化循环场景（darwin-skill 用法）**强制**跑——这是 §12 无效改进识别的前提
- 准入评级场景**推荐**跑（若 subagent 不可用，标注"未跑 baseline 对照"，并在 §12 触发"无效改进检测不可用"警告）

**降级**：若 subagent 不可用（超时/资源限制），退化为"干跑验证"——读完 skill 后模拟一个典型 prompt 的执行思路，判断流程是否合理；必须在报告中标注 `dry_run`。dry_run 比例 > 30% → 该增强项失效警告，且 §12 多条规则失去输入（darwin-skill 实证：dim8 权重 23% 时无 full_test 验证分数不可信）。

> 本子项让 B3 从"输出绝对质量"扩展到"skill 增益证明"，与 B3.1-B3.5 互补：B3.1-B3.5 评"输出好不好"，B3.6 评"skill 贡献了什么"。**v1.2 起为"得分虚高"问题的核心判据**——结构分再高，若 B3.6 显示持平/退步，则触发 §12 无效改进识别。
...
### 5.4 效果子评级（v1.2 新增，对抗"得分虚高"）

**为什么需要**：总分由 Tier A 静态（50）+ Tier B 运行时（50）构成，但**静态分易通过结构改动凑涨**（加 checkpoint 标记、补 fallback 表、去模糊词——这些低成本改动能稳定涨分），而**真正反映 skill 效果的运行时维度（B1 触发精度 / B3 输出质量 / B4 鲁棒性）难涨**。这导致优化循环常出现"总分涨了 X 分但实际效果没变"的虚高现象。

**效果分公式**：

```
效果分 = B1 + B3 + B4 + B3.6量化增量
       = 10 + 10 + 10 + {-3..+3}
       = 27 ~ 33 分（理论区间）
```

- B1（触发精度）：skill 该触发时触发、不该触发时不触发
- B3（输出质量）：输出实际好不好
- B4（鲁棒性）：边缘/对抗/异常场景表现
- B3.6 量化增量：与不带 skill 的 baseline 对照证明 skill 是否真带来增益
...
|---|---|---|
| ≥28 | E-A | skill 实际效果优秀 |
| 24-27 | E-B | skill 实际效果合格 |
| 20-23 | E-C | skill 实际效果一般 |
| <20 | E-D | skill 实际效果不足 |

**与总分的关系**（v1.2 关键）：
- 总分 ≥ A（85）但效果分 ≤ E-C（≤23）→ 触发 **§12 无效改进识别**，报告中显式标"总分虚高，实际效果未达 A 级"
- 效果分 E-D（<20）→ 总分封顶 84（不得评 A 级），无论结构分多高
- 准入评级场景：效果分必须 ≥ E-B（24）才推荐发布

**dry_run 降级算法**（v1.3 明文化）：

B3.6 在 subagent 不可用时会 dry_run（仅"干跑验证"未实际 spawn baseline 对照）。此时效果分算法必须明文调整：

| B3.6 状态 | 效果分算法 | 理论区间 | 可信度 |
|---|---|---|---|
| full_test（实际跑了 baseline） | B1 + B3 + B4 + B3.6量化增量 | 27 ~ 33 | 高 |
| dry_run（未跑 baseline） | **B1 + B3 + B4**（不加 B3.6 增量，B3.6 增量记 0） | 0 ~ 30 | 中 |
| 未尝试（B3.6 跳过） | B1 + B3 + B4 | 0 ~ 30 | 低 |

**可信度三档标识**（v1.3，必须标注在效果分后）：

| 可信度 | 判定 | 报告处理 |
|---|---|---|
| **高** | B3.6 full_test + paired 实跑 + §12 规则 5 未触发 | 效果分直接用于决策 |
| **中** | B3.6 dry_run，或 §12 规则 5 触发（已降权处理） | 效果分用于决策但标注"未含 baseline 佐证"，建议补跑 |
| **低** | B3.6 未尝试，或 §12 规则 4 触发（dry_run 比例 > 30%），或 §12 触发 ≥2 条 | 效果分仅作参考，**不可单独作为 keep/revert 依据**，必须配合人工审查 |

> bug-analyzer 测评报告（v1.2）实际已采用此算法：B3.6 dry_run → 效果分 = B1+B3+B4 = 9+10+9 = 28，但当时方法论未明文规定。v1.3 把这个实践上升为规范。

**适用场景**：优化循环（darwin-skill 场景）必算；准入评级场景推荐算（若 B3.6 dry_run，按上表算法 + 标可信度"中"）。
...
| **delta 决策用 within-judge paired** | 改进前后对比 / 版本对比 / A vs B 选优等**delta 决策场景**：同一 judge 在【同一次 call 内】读两版，投 better/worse/tie + margin{clear\|slight} + 一句理由 + **改后版潜在退化场景**（v1.2 必填，见下）；取奇数 N（默认 3，close call 升 5）多数决。**绝对分数仅做 triage**（粗排"先改谁"），不用于 keep/revert 等delta 决策 | darwin-skill v2.1：绝对分跨 judge ±8 噪音淹没保守编辑的 +3~8 真实增益；within-judge cancellation 让"不准的尺对两版等量作用、比较时抵消"。注：与 COLM 的 across-judge pairwise 不冲突——后者换尺污染未消除，前者通过 within-judge 消除 |
| **改后版潜在退化场景必填**（v1.2） | paired 投 better 时，judge **必须**列出 ≥1 个"改后版可能更差的场景"（如"加 checkpoint 后，简单任务流程变冗长"）。列不出 → 视为 judge 乐观偏差，该票降权（margin clear → slight）或作废 | SkillLens 实证 LLM judge 乐观准确率 46.4%（低于悲观的 76.2%）；darwin 自身反例第 3 条"为凑分增冗余"即是症状。强制列退化场景是对抗 judge 乐观偏差的工程化手段 |
| **退化场景两要素质量标准**（v1.3） | 退化场景必须同时含：①**具体场景**（哪个 prompt / 哪类输入 / 哪个执行路径）②**为何更差**（如"流程冗长增加 N 步"、"条件分支误判类"、"新增校验误拦合法输入"）。**只有其一 = 不合格**，按"列不出"处理 | v1.2 仅说"列 ≥1 个退化场景"但无质量门槛，judge 易用"流程变冗长"等空泛话术敷衍。两要素强制把退化场景从"凑数"升级为"可验证" |
| **降权逻辑明文化**（v1.3） | judge 列不出合格退化场景时：①该 judge 票 **better 降权为 tie**（非作废——作废会减少多数决基数）②若该轮 >50% judge 触发降权 → **该轮整体判定退为 tie**（不再算 keep）③连续 2 轮该轮退为 tie → **强制 close call 升 N=5 重投**，仍退为 tie → 判定"持平"非"改进" | v1.2 的"降权或作废"未定义具体逻辑：作废会减少 N 导致多数决失稳；统一降为 tie 既保留基数又抑制乐观偏差。连续 2 轮退为 tie 是"反复列不出退化场景"的硬信号 |
...
### 疑似无效改进（v1.2 新增，仅优化循环场景适用）

若本次测评为"优化前后对比"场景，且 §12 任一检测规则触发，必须在此显式列出：

| 触发规则 | 触发值 | 含义 |
|---|---|---|
| {规则编号} | {触发值，如"结构分涨 6 / 效果分涨 1 = 比值 6.0"} | {一句话说明，如"结构凑分嫌疑"} |
...
- **疑似无效改进仅在优化循环场景输出**（v1.2）：准入评级场景无需此段；优化循环场景若 §12 触发则必须列，未触发也需明确标注"无嫌疑"
...
## 效果子评级（v1.2）
- 效果分：{X}/33（B1+B3+B4+B3.6量化增量）
- 效果子评级：{E-A/E-B/E-C/E-D}
- 总分 vs 效果分一致性：{一致 / 总分虚高 / 效果分不足}
- 若总分≥A 但效果分≤E-C：标注"总分虚高，实际效果未达 A 级"

## 无效改进检测（v1.2，仅优化循环场景）
- §12 触发规则：{无 / 列出规则编号}
- 触发结论：{无嫌疑 / 疑似无效改进 / 实际退步}
- 若触发，列出触发值与建议
...
## 12. 无效改进识别（v1.2 新增，针对"得分虚高但实际效果不提升"问题）

### 12.1 问题根因

darwin-skill 等优化器在实际使用中常出现"总分涨了 X 分但实际效果没变"的虚高现象，根因有六：

| # | 根因 | 机制 |
|---|---|---|
| 1 | **结构分与效果分脱钩** | 总分含 50 分静态分（A1-A5）+ B2/B5，这些维度易通过结构改动凑涨（加 checkpoint 标记、补 fallback 表、去模糊词——低成本高涨幅）；而真正反映效果的是 B1/B3/B4，难涨。darwin HL-1 实证"4 行改动撬动 dim4 +3 分"——结构改动投入产出比远高于效果改动 |
| 2 | **LLM judge 乐观偏差** | SkillLens 实证 LLM judge 乐观准确率 46.4%（vs 悲观 76.2%），倾向"改后版更详细=更好"。加了 checklist/fallback 表后 judge 觉得"更完善"打高分，但这些 additions 是否真的提升了 skill 在真实场景的表现？未验证 |
| 3 | **baseline 对照常 dry_run 失效** | darwin 自承早期 40 次记录 67% dry_run。dim8 权重 23% 却照打分，分数虚高但无 full_test 验证 |
| 4 | **加权短板贪心导致"修易不修难"** | weighted_gap = weight × (10 - score) / 10 优先修权重高的短板。但权重高的维度（如 darwin dim5=18 可执行具体性）不一定是实际效果最相关的维度。修 dim5 涨分快，但 dim5 涨了是否真的让 skill 效果更好？不一定 |
| 5 | **缺真实任务 ground truth** | test-prompts.json 的 expected 字段常为人写的简短描述，不是真实任务的 ground truth。skill 可能产出符合 expected 描述但实际无用的输出（如 expected 写"输出根因"但实际根因错误也算通过） |
| 6 | **paired judge 仍同模型** | within-judge cancellation 消除换尺污染，但 judge 模型本身的乐观偏差未消除。judge 可能系统性倾向"改后版"（看起来更详细/规范） |

### 12.2 六条检测规则

针对上述根因，定义六条无效改进检测规则。任一触发 → 报告中显式标注，并按 §12.3 处理。

#### 规则 1：结构凑分检测（根因 1）

```
触发条件：结构分涨幅 / 效果分涨幅 > 2.0 且 效果分涨幅 < 3
```

- 结构分 = A1 + A2 + A3 + A4 + A5 + B2 + B5（共 70 分）
- 效果分 = B1 + B3 + B4（共 30 分，不含 B3.6 增量）
- 触发含义：结构改动占主导，效果未实质提升
- 例：结构分涨 6 / 效果分涨 1 → 比值 6.0，触发"疑似结构凑分"
...
- 体积 = 入口文件 + 触发加载的知识文件总行数
- 触发含义：为涨分堆冗余内容（darwin 反例第 3 条症状）
- 例：体积涨 50% / 总分涨 3 → 比值 16.7，触发"疑似冗余凑分"
- **例外可追溯标准**（v1.3 明文化）：若新增内容**可追溯到以下评分项**，不算冗余，规则 2 不触发：
  - **A4.3 Fallback**（降级路径与错误处理）
  - **B4.2 corrupt success**（异常输出检测）
  - **A5 反例与黑名单**（反模式条目 + 替代做法）
  - **B5.4 高风险动词黑名单**（动词列禁 + 安全写法）
  - **A3.3 模糊词黑名单**（被扫描并替换的具体词）
  
  判定方法：新增内容必须**同时满足**：①在该评分项下打分涨了 ②新增的每段内容能指出对应评分项编号 ③新增内容含具体的可执行信息（如"用 X 替代 Y"而非"应该注意 X"）。三者缺一即算冗余。
  
  反例（不算例外，仍触发规则 2）：
  - 新增"建议在合适时使用 X"（无具体场景，A3.3 模糊词）
  - 新增"考虑各种边界情况"（无具体边界列表，A4.3 无评分涨幅）
  - 新增"参见某论文"（无具体引用位置与论点对应，A5 无评分涨幅）

#### 规则 3：baseline 退步检测（根因 3，硬触发）

```
触发条件：B3.6 增益等级 = 退步
```

- 触发含义：skill 实际效果比不带 skill 还差——这是**硬触发**，直接判"实际退步"
- 处理：无论总分涨多少，本次改进判定为**失败**，建议 revert
- 软封顶：总分封顶 59（D 级），不得评 A/B/C

#### 规则 4：dry_run 失效检测（根因 3）

```
触发条件：B3.6 dry_run 比例 > 30%
```

- 触发含义：baseline 对照未实际跑，B3.6 分数不可信
- 处理：效果子评级标注"未含 baseline 对照"，且 §12 规则 1/2/3 失去 B3.6 输入，仅规则 5/6 可用
- 警告：darwin 实证 dim8 权重 23% 时无 full_test 验证分数不可信

#### 规则 5：paired 乐观偏差检测（根因 2/6）

```
触发条件：paired 比较中 margin=slight 占比 > 60% 且 better 占比 ≥ 2/3（v1.3 放宽）
         （v1.2 原条件：better 数 = N，即全员一致才触发）
```

- 触发含义：多数 judge 认为改后版"略好"但都是 slight margin——可能是乐观偏差而非真实改进
- v1.3 放宽理由：v1.2 要求 `better = N`（全员一致）才触发，但典型乐观偏差场景是 `2B + 1T` 或 `2B + 1W`（2 人略好 + 1 人持平/退步）——这恰是 v1.2 漏判的高发区。把"全员一致"放宽为"2/3 多数"，覆盖率从 ~30% 提升至 ~70%，假阳性代价低（触发后只是多审退化场景）
- 处理：触发 §7"改后版潜在退化场景必填"项的强制执行；judge 列不出合格退化场景（两要素标准 v1.3）→ 该票降权为 tie（v1.3 降权逻辑）；连续 2 轮该轮退为 tie → close call 升 N=5 重投
- 升级：close call 升 N=5 重投，若仍 slight 占比 > 60% 且 better 占比 ≥ 2/3 → 判定为"持平"而非"改进"
- N=3 下触发组合矩阵（v1.3）：

| 投票组合 | better 占比 | slight 占比 | v1.2 | v1.3 | 说明 |
|---|---|---|---|---|---|
| 3B(3 slight) | 100% | 100% | ✅ | ✅ | 全员略好 |
| 3B(2 slight + 1 clear) | 100% | 67% | ✅ | ✅ | 全员好，2 略好 |
| **2B(2 slight) + 1T** | 67% | 67% | ❌ | ✅ **新增** | 多数略好，1 人持平——典型乐观偏差 |
| **2B(2 slight) + 1W** | 67% | 67% | ❌ | ✅ **新增** | 多数略好，1 人退步——退化被掩盖 |
| 2B(1 slight) + 1T | 67% | 33% | ❌ | ❌ | slight 占比 < 60% |
| 2B + 1W(全 clear) | 67% | 0% | ❌ | ❌ | slight 占比 0% |

#### 规则 6：expected 字段过弱检测（根因 5）

```
触发条件：test-prompts.json 中 expected 字段平均长度 < 20 字符 或 含"正常/正确/合理"等不可判定词
```

- 触发含义：测试集 ground truth 过弱，B3.1/B3.2 评分可能虚高
- 处理：测试集质量降级警告；建议补强 expected 为"可判定的具体行为"（如"输出包含 trace_id 字段且值非空"而非"输出正确"）
- 影响：B3 维度评分置信度降为"低"
...
| 触发规则数 | 处理 |
|---|---|
| 0 | 无效改进嫌疑：未触发任何规则，改进可信 |
| 1（非规则 3） | 疑似无效改进：报告中标注触发规则与触发值，改进仍保留但标注"需后续验证" |
| ≥2（非规则 3） | 高度疑似无效改进：改进保留但总分涨幅折扣 ×0.5（如总分涨 6 → 实际计入涨 3） |
| 任一规则 3 触发 | 实际退步：改进判定失败，建议 revert；总分软封顶 59 |

### 12.4 适用场景

- **优化循环场景**（darwin-skill 用法）：**强制**跑 §12 全部检测，作为 keep/revert 决策的硬约束之一
- **版本对比场景**（v1.2 比 v1.1 好吗）：**推荐**跑规则 1/2/3/5（规则 4/6 依赖测试集与 baseline 数据，若无可省）
- **准入评级场景**（一次性评级）：**不跑**（无前后对比基准）；但若跑过 baseline 对照，规则 4 仍可作置信度警告

> v1.3 补充：§12 触发结果直接影响 §5.4 效果子评级的可信度标识（高/中/低三档）——见 §5.4 可信度三档表。

### 12.5 与 §5.4 效果子评级的关系

- §5.4 效果子评级是 §12 的**前置计算**——规则 1/3/4 依赖效果分与 B3.6 增量
- §12 是 §5.4 的**应用层**——当效果分与总分不一致时，§12 给出具体的"为什么"与"怎么办"
- 两者协同：§5.4 检测"虚高"，§12 诊断"虚高的原因并处理"
- v1.3 新增联动：§12 触发条数 → §5.4 可信度档位（0 条 = 高，1 条 = 中，≥2 条 = 低）
