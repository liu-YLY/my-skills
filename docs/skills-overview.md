...
| 混合意图（7 条链） | 见 SKILL.md 混合意图链 | 转交披露依据，按既有授权继续 |
...
**工作流程**：
```
阶段 1: 项目特征 → 阶段 2: 风险矩阵 → 阶段 3: 测试分层 → 阶段自检 → 阶段 4: 范围准入准出 → 阶段 5（可选）: 资源附录
```
...
AI（test-case-engineer）：
→ 触发评审模式，不走四阶段生成流程
→ 按 10 维度评审：覆盖度、优先级合理性、字段规范、可执行性、冗余、溯源、可维护性、可自动化、测试数据依赖、语义一致性
...
**工作流程**：
```
阶段 1: 需求理解 → 阶段 2: 场景设计 → 阶段自检 → 阶段 3: 瓶颈定位 → 阶段 4: 转交判断
```
...
**工作流程**：
```
阶段 1: 状态型需求识别 → 阶段 2: 状态机建模 → 阶段自检 → 阶段 3: 完整性检查 → 阶段 4: 10 类场景穷举 → 阶段 5（可选）: MCP 增强
```
...
**工作流程**：
```
阶段 1: 收集输入 → 阶段自检 → 阶段 2: Diff 解析 → 阶段自检 → 阶段 3: 交叉分析 → 阶段 4: 生成报告
```
...
**版本**：v3.0.2
**功能**：微信公众号文章排版技能，提供多种适用于互联网/技术领域的排版风格模板和高级排版模块。自动分析用户文章内容，按选定风格完成排版，支持 `:::module` 语法创建专业视觉卡片组件，输出为可直接复制到公众号编辑器的格式化 Markdown 文件。

**核心能力**：
- **内容分析**：自动识别文章要素（标题层级、代码块、列表、要点）
- **风格匹配**：提供 6 种专业排版风格，智能推荐 2-3 种最合适的风格供用户选择
- **排版执行**：严格按模板规则转换，确保格式一致性
- **高级排版模块**：`:::module` 语法支持 9 大类预定义视觉卡片组件（hero/toc/cards/verdict/cta/steps 等）
- **品牌配置**：支持 Brand Profile 文件（全局 `~/.config/md2wechat/brand.md` 或项目 `.brand.md`），统一视觉风格与品牌调性
- **质量校验**：覆盖度 + 可读性 + 公众号兼容性三项检查
- **HTML 生成**：仅当用户明确要求「HTML / 可直接粘贴 / 一键复制」时生成带内联样式的 HTML 文件
