...
> **降级策略**：若该共享文件不存在（bug-analyzer 单独安装场景），以下方「内容速览」的模式分类为线索，结合通用测试经验继续分析，不中断流程；并在输出中注明「未查阅完整缺陷模式库，分析基于通用模式」。
...
- 安全专项检查清单（10 项）
- 「功能特征 → 缺陷模式」自动映射表
- **运行时高频错误类型 checklist**：NPE / 500 错误 / 超时（含典型症状、根因分类、排查路径、验证方法）
