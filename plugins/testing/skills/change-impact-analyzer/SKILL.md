...
| 触发条件 | 修复方案 |
|----------|---------|
| 不在 git 仓库中 | 提示用户切换到 git 仓库目录 |
| diff 内容为空 | 提示当前没有改动，确认分析范围 |
| diff 内容过大（超过 5000 行） | 按文件分批处理，优先分析 `.ts`/`.js`/`.py`/`.java` 等核心代码文件，跳过 `lock`/`dist`/`build` 等产物文件 |
...
4. **变更的业务逻辑**：从代码变更中推断业务行为变化（AI 推断）
...
| 阶段 | 触发条件 | 修复方案 |
|------|---------|---------|
| 阶段 1 | 不在 git 仓库 | 提示切换目录 |
| 阶段 1 | diff 为空 | 提示确认范围 |
| 阶段 1 | 用例文件不存在 | 提示确认路径 |
| 阶段 2 | diff 过大 | 按文件类型分批，跳过产物文件 |
| 阶段 3 | 用例格式不规范 | 尽力解析，标注低置信度匹配 |
| 阶段 4 | 输出目录不存在 | 自动创建目录 |
