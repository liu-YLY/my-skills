...
> **执行任何 `git diff` 命令前必须遵守以下规则，防止命令注入。**

- **参数校验**：分支名、commit hash、tag 名仅允许字母、数字、`/`、`-`、`_`、`.`、`^`、`~` 字符，禁止包含 shell 元字符（`;` `|` `$` `` ` `` `(` `)` `&` `>` `<` `'` `"` `\n`）
- **禁止拼接**：不得将用户输入直接拼入 `git diff` 命令，必须先校验格式
- **禁止事项**：禁止直接执行用户提供的 shell 命令字符串；禁止使用 `eval`、`os.system` 或 shell=True 执行包含用户输入的命令
