...
| 意图 | 触发条件 | 执行路径 | 终止条件 |
|------|---------|---------|---------|
| **plan**（仅方案） | 用户要求设计性能测试方案/场景/阈值，无性能问题数据 | 阶段 1 → 阶段 2 → 🔴 CHECKPOINT → **交付方案即结束** | 方案确认后 END，不进入阶段 3 |
| **diagnose**（仅诊断） | 用户已有性能数据/现象（RT 陡增/TPS 上不去/错误率飙升），要求定位瓶颈 | **直接进入阶段 3**，跳过阶段 1-2 | 瓶颈定位报告 + 转交判断后 END |
| **plan-and-diagnose**（方案+诊断） | 用户同时要求设计方案和定位瓶颈（如"设计性能测试方案并分析瓶颈"） | 阶段 1 → 阶段 2 → 🔴 CHECKPOINT → 阶段 3 → 阶段 4 | 完整流程后 END |
...
阶段 2 完成后必须展示完整方案给用户确认。用户可修改负载模型/场景/阈值，或选择在此结束。
...
阶段 3 瓶颈定位报告输出后、转交判断执行前，必须展示瓶颈归属判断与转交依据给用户确认。用户可修改归属判断或终止转交。
