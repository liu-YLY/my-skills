...
```
阶段 1 性能需求理解 → 阶段 2 测试场景设计 → 方案自检 → 阶段 3 瓶颈定位 → 阶段 4 转交判断
```
...
| 意图 | 触发条件 | 执行路径 | 终止条件 |
|------|---------|---------|---------|
| **plan**（仅方案） | 用户要求设计性能测试方案/场景/阈值，无性能问题数据 | 阶段 1 → 阶段 2 → 方案自检 → **交付方案即结束** | 方案交付后 END，不进入阶段 3 |
| **diagnose**（仅诊断） | 用户已有性能数据/现象（RT 陡增/TPS 上不去/错误率飙升），要求定位瓶颈 | **直接进入阶段 3**，跳过阶段 1-2 | 瓶颈定位报告 + 转交判断后 END |
| **plan-and-diagnose**（方案+诊断） | 用户同时要求设计方案和定位瓶颈（如"设计性能测试方案并分析瓶颈"） | 阶段 1 → 阶段 2 → 方案自检 → 阶段 3 → 阶段 4 | 完整流程后 END |

> **终止原则**：用户目标已满足时停止，不强制进入后续阶段。仅要方案时方案交付即 END；仅要诊断时诊断报告交付即 END。

**阶段间依赖与分支入口**：
- 阶段 2 的输入是阶段 1 的性能需求摘要
- 阶段 2 完成后检查方案依据；已有诊断数据和授权时直接继续
- 阶段 3 为逆向触发：当输入含性能数据/现象（RT 陡增/TPS 上不去/错误率飙升）时，**直接进入阶段 3，跳过阶段 1-2**
...
1. 负载模型选择，参考 [knowledge/load-models.md](knowledge/load-models.md) 决策表：
   - 阶梯递增 → 找性能拐点（默认推荐，验证预期负载下能否达标）
   - 峰值 → 验证瞬时冲击承受力（找系统崩溃临界点）
   - 疲劳 → 暴露泄漏与慢查询累积（长时间运行验证内存泄漏/资源耗尽）
   - 容量 → 量化最大处理能力（找系统能承载的最大用户/TPS）
   - 基线场景（可选）→ 单用户基线性能，非独立负载模型
2. 场景设计：阶梯递增场景必选；若需验证基线加基线场景，若需找崩溃临界点加峰值场景，若需验证长时间运行加疲劳场景
...
阶段 2 完成后交付方案及假设；只读分析无需重复确认。实际压测、配置修改及未授权操作另行核对范围。

展示内容：
- 选定的负载模型及理由
- 场景清单（基线/阶梯递增/峰值/疲劳/容量，按选定模型列举）
- 指标阈值表（TPS / RT 分位数 / 错误率 / 资源利用率，均为具体数值）

**用户选项**：
- 修改方案 → 修改后重新确认
- 确认方案，继续瓶颈定位 → 进入阶段 3（plan-and-diagnose 意图）
- 确认方案，到此结束 → **交付方案，END**（plan 意图）

有现成性能证据时继续只读诊断；实际压测和修改按当前授权范围执行。
...
2. 按最强证据选择起点并交叉验证；无明显线索时检查应用、资源、架构，允许按证据调整：
...
展示瓶颈归属及转交证据；已授权的只读诊断继续执行。缺少关键信息、改变范围或执行未授权操作时再询问。
...
| 瓶颈指向 | 定位层 | 处理方式 | 转交依据 |
|----------|--------|---------|---------|
| 资源/架构层（连接池不足、扩容需求、缓存未命中、DB 分库分表、限流缺失） | 资源/架构层 | 本 skill 给优化建议 | — |
| 代码逻辑缺陷（死锁、内存泄漏、N+1 查询、锁竞争、算法低效） | 代码逻辑层 | 转交 bug-analyzer | 携带瓶颈定位报告 + 已收集性能数据 + 怀疑的代码模块 |

**层边界对照**（本 skill vs bug-analyzer）：
- 输入差异：本 skill 吃性能数据（CPU/IO/RT/TPS）；bug-analyzer 吃 Bug 现象/错误日志/复现步骤
- 方法论差异：本 skill 用 USE 方法/资源三角/瓶颈传导链；bug-analyzer 用 5 Whys/鱼骨图/隔离定位
- 输出差异：本 skill 给瓶颈点 + 资源/架构优化建议；bug-analyzer 给根因 + 代码修复建议

**输出**：处理结论（本 skill 优化建议 / 转交 bug-analyzer）。
...
| 3 | **无证据确定瓶颈层** | 固定顺序或单一指标不能证明因果 | 根据证据选择起点，交叉核对应用、资源和架构指标 |
...
2. **按授权与业务依赖继续** — 缺少影响结论的关键信息、范围变化或未授权写操作时询问
3. **定位顺序依据证据调整** — 用指标和因果链支持归属，不凭固定顺序确认根因
4. **指标注明依据** — 区分实测、目标与估算；缺 SLA 或校准数据时标为待定，不编造数值
