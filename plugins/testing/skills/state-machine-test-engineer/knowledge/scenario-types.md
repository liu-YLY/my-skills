...
**生成条件**：transition 涉及管理员/审批人/特定角色操作，或目标态隐含角色权限。若 PRD 未明确权限矩阵或场景无角色信号，全部标"待确认"并给出 `expected_target_state_reason`（说明待确认原因），不静默跳过。
...
**生成条件**：所有 transition 都生成。PRD 通常未写失败后状态，多数标"待确认"，须随附 `expected_target_state_reason`。
