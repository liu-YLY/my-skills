...
- 阶段 5 为可选触发：仅当用户明确要求"资源估算/进度排期/风险跟踪"时执行，否则跳过
...
**触发条件**：用户明确要求"资源估算/进度排期/风险跟踪"。不触发时跳过，并在策略文档末尾标注"未生成（用户未要求）"。

**输入**：阶段 2-3 的风险矩阵 + 分层策略 + 阶段 4 的范围
**动作**：
1. 资源粗估：按各层测试用例数 × 单用例工时估算人天，参考 [knowledge/strategy-templates.md](knowledge/strategy-templates.md) 附录模板
