...
2. 按 5 维评估每个模块，参考 [knowledge/risk-matrix-framework.md](knowledge/risk-matrix-framework.md)：
   ...
   - 外部依赖
   - 数据敏感性
3. 有事实依据的维度按 1-5 分量化；任一维度缺少事实时标「待补」，不计算正式 5 维总分，可另列有依据的临时优先级与置信度
...
**动作**：
1. 资源粗估：优先使用团队历史基线；没有基线时用区间和显式假设估算，不把模板工时写成承诺值，参考 [knowledge/strategy-templates.md](knowledge/strategy-templates.md) 附录模板
