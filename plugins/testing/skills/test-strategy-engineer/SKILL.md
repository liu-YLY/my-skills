...
- 阶段 3 先自检风险依据与分层职责，无阻塞信息则继续阶段 4
...
1. 按项目类型选择分层比例，参考 [knowledge/test-pyramid.md](knowledge/test-pyramid.md)：
   - 新项目 → 经典金字塔（单元 60-70% / 接口 20-30% / UI 5-10%，数值以 test-pyramid.md 为准）
   - 迭代项目 → 橄榄型（接口层加厚，单元层维持）
   - 重构项目 → 倒金字塔过渡（先补接口/UI 兜底，再补单元）
   - 迁移项目 → 沙漏型（接口对比层为主）
   - **维度关系**：形状由项目生命周期（新/迭代/重构/迁移）决定倾向，具体比例数值由技术栈分档表（test-pyramid.md §3：后端/前端/全栈/微服务/遗留）决定，两者正交组合——先定形状，再查表取比例
...
阶段 3 展示风险矩阵与分层依据，继续已有授权内的方案设计。仅缺关键信息、范围改变或未授权写操作时询问。
...
用户要求逐阶段审阅时保留确认点；其余情况在交付时汇总待确认项。
...
3. 制定准入标准，参考 [knowledge/entry-exit-criteria.md](knowledge/entry-exit-criteria.md)：需求评审通过、单元测试覆盖率 ≥ 70%（核心模块 ≥ 80%）、构建产物可部署、测试环境就绪
4. 制定准出标准：用例执行率 ≥ 95%、需求覆盖率 ≥ 100%、P0/P1 缺陷修复率 100%、P2 缺陷修复率 ≥ 95%、性能达标、回归通过（详见 [knowledge/entry-exit-criteria.md](knowledge/entry-exit-criteria.md) §2）
...
阶段 4 将项目已确认阈值与模板建议分开列出。缺少业务依据时交付待确认建议及影响，不因尚未审核而扣留策略文档。

策略交付不代表发布批准；实际执行或放行以用户授权和项目已确认门槛为准。
