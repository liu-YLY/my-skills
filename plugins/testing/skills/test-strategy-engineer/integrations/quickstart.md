...
- 阶段 3 测试分层策略：新项目 → 经典金字塔（单元 60-70% / 接口 20-30% / UI 5-10%，数值以 test-pyramid.md 分档表为准），高风险模块三层全覆盖，中风险模块单元+接口，低风险模块仅接口
...
- 分层策略：经典金字塔（单元 60-70% / 接口 20-30% / UI 5-10%），支付与订单三层全覆盖
