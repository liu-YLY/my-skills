...
> 微信公众号排版 plugin，针对互联网 / 技术领域文章提供 6 种专业排版风格、高级排版模块与可选的 HTML 一键生成能力。
>
> 自动分析文章内容（编程知识、测试理论、架构设计、技术分享等），匹配最佳风格，默认输出可直接复制到公众号编辑器的格式化 Markdown；用户明确要求时再生成可粘贴 HTML。

## 版本

**当前版本**：v3.0.2（plugin manifest / skill frontmatter 同步）
...
| **HTML 生成（可选）** | 仅用户明确要求「HTML / 可直接粘贴 / 一键复制」时，生成带内联样式与"复制"按钮的 HTML，可直接粘贴到公众号 |
...
├── README.md                         # 本文件
└── skills/
    └── wechat-formatter/             # 排版 skill v3.0.2
        ├── SKILL.md                  # 入口 + 五阶段流程
        ├── README.md                 # skill 说明
        ├── test-prompts.json
        ├── brand/                    # 品牌配置
