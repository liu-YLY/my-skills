...
### 用法 1：默认模式（自动分析 + 推荐，默认输出 Markdown）
...
7. **默认到此结束**：输出格式化 Markdown 文件
8. **可选**：用户明确要求「HTML」「可直接粘贴」「一键复制」时，AI 执行阶段 5 调用 `md2wechat.py` 生成 HTML 文件，用户在浏览器打开后点击「复制到公众号」按钮粘贴到微信编辑器
...
AI：（完成排版，输出格式化 Markdown 文件路径；如用户明确要求 HTML，再生成 HTML 文件）
```

**对应五阶段**：默认执行阶段 1 → 2 → 3 → 4；阶段 5 仅在用户明确要求 HTML 时执行
...
5. 默认输出格式化 Markdown；**仅在用户明确要求 HTML 时**执行阶段 5
...
AI：（跳过推荐，直接完成排版，输出 Markdown 文件路径；如需 HTML，请明确告知）
```

**对应五阶段**：阶段 1（简版）→ 跳过阶段 2 → 阶段 3 → 阶段 4（简化）；阶段 5 仅在用户明确要求时执行
...
| 阶段 5 生成 HTML | ⚙️ 可选（用户明确要求时） | ⚙️ 可选（用户明确要求时） | ❌ 跳过 |
