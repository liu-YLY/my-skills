...
> **何时阅读**:仅在定位 Python 代码缺陷 / 审查 Python 代码 / 写 pytest 测试时查阅。
> **覆盖范围**:数据类型陷阱(可变默认参数/浅深拷贝/is vs ==)+ 异常处理 + 并发与线程安全 + Web 框架(Flask)常见问题 + API 测试 + 日志/堆栈/网络/DB/性能排查工具 + pytest 要点(参数化/fixture/mock)。
> **可跳过条件**:本次任务不涉及 Python 代码;不在写 pytest 自动化。
> **快速定位**:
